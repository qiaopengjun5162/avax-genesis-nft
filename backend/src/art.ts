...
export function genesisArt(wallet: string, index: number): string {
  ...
  const svg =
    ...
    `<text x="200" y="372" text-anchor="middle" font-family="monospace" font-size="18" fill="#ffffff" opacity="0.45">${norm.slice(0, 6)}...${norm.slice(-4)}</text>` +
    `</svg>`;
